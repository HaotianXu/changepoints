...
sigma = 1
gamma_set = c(0.01, 0.05, 0.1, 0.5, 1:16)
# kappa_mat = cbind(c(3, 9, -27), c(0, 710.0551, 0)) # jump sizes of coefficients for reparametrized model
# (length(cpt_true)*sigma^2*log(n))^(-1) * 3^3 * n
 kappa_mat = cbind(c(3, 9, -27), c(0, 0, 168059.4)) # jump sizes of coefficients for reparametrized model
# (length(cpt_true)*sigma^2*log(n))^(-2) * 3^5 * n^2
init_coef_vec+kappa_mat[,1] # coefficients after the first changepoint
coef.repara(init_coef_vec+kappa_mat[,1], cpt_true[1], cpt_true[2], n) # reparametrized coefficients after the first changepoint
coef.repara(init_coef_vec+kappa_mat[,1], cpt_true[1], cpt_true[2], n)+kappa_mat[,2] # coefficients after the second changepoint
...
save(r, init_coef_vec, cpt_true, effect_size, n, sigma, kappa_mat, cpt_est_init, error_init, len_est_init, cpt_est_lr, error_lr, len_est_lr, scaled_Hdist_init, scaled_Hdist_lr, file = "C:/Users/haotian/Documents/GitHub/changepoints/examples/poly_r2_n300_equalspace_001_extra.RData")


##############################################################
# Simulation n = 450 Extra
RR = 100
len_est_init = rep(0, RR)## number of estimated change points
cpt_est_init = vector("list", RR) ## estimated change points
error_init = rep(0, RR)
...
sigma = 1
gamma_set = c(0.01, 0.05, 0.1, 0.5, 1:16)
# kappa_mat = cbind(c(3, 9, -27), c(0, 994.3941, 0)) # jump sizes of coefficients for reparametrized model
# (length(cpt_true)*sigma^2*log(n))^(-1) * 3^3 * n
kappa_mat = cbind(c(3, 9, -27), c(0, 0, -329606.6)) # jump sizes of coefficients for reparametrized model
# (length(cpt_true)*sigma^2*log(n))^(-2) * 3^5 * n^2
init_coef_vec+kappa_mat[,1] # coefficients after the first changepoint
coef.repara(init_coef_vec+kappa_mat[,1], cpt_true[1], cpt_true[2], n) # reparametrized coefficients after the first changepoint
coef.repara(init_coef_vec+kappa_mat[,1], cpt_true[1], cpt_true[2], n)+kappa_mat[,2] # coefficients after the second changepoint
...
save(r, init_coef_vec, cpt_true, effect_size, n, sigma, kappa_mat, cpt_est_init, error_init, len_est_init, cpt_est_lr, error_lr, len_est_lr, scaled_Hdist_init, scaled_Hdist_lr, file = "C:/Users/haotian/Documents/GitHub/changepoints/examples/poly_r2_n450_equalspace_001_extra.RData")
